  ...
  <div class="mt-3 mx-8 border-b pb-5">
    <div class="container flex-col sm:flex-row">
      <div class="flex flex-1 flex-col justify-center sm:mr-3">
        <h2 class="text-4xl font-semibold text-left mb-3">Generate color scheme from photos and use them in TailwindCSS
        </h2>
        <h3 class="text-xl text-left mb-8 text-gray-700">
          Upload your image and our tool will generate the right palettes for you to use in your project with
          TailwindCSS.
        </h3>
        <div class="flex items-center flex-col lg:flex-row justify-center lg:justify-start">
          <button
            class="rounded px-5 py-3 bg-cloudinary-500 text-white relative mb-3 w-full lg:w-auto flex justify-center">
            <span class="flex items-center">
              <svg-icon name="upload" class="w-4 h-4 fill-current mr-3" />
              Upload image
            </span>
            <input type="file" tabindex="-1" accept=".jpeg,.jpg,.png,image/jpeg,image/png"
              class="opacity-0 absolute inset-0 w-full cursor-pointer" @change="selectFile"
      ...
      <div v-show="loading"
        class="w-full sm:w-box h-box flex flex-col justify-center items-center border rounded-t-lg rounded-b-lg bg-gray-100">
      ...
      <div class="rounded-b-lg rounded-t-lg relative sm:w-box sm:h-box flex" v-show="!loading">
        <picture class="flex items-center">
          ...
          <img :src="demo.delivery_url" alt="uploaded photo for color detection"
            class="rounded-t-lg rounded-b-lg self-align sm:w-box" />
        </picture>
        <div class="rounded-b-lg flex h-24 absolute bottom-0 w-full">
          <div v-for="(color, index) in demo.colors" :key="color.main.hex"
    ...
    <div class="border-t mt-24 py-5" v-if="!loading">
      ...
      <p class="text-lg pb-5 text-gray-700 text-center italic">Customize the color theme for your TailwindCSS project
        <b>in 3 simple steps</b>:</p>
      <p class="text-md py-5 text-gray-800 flex justify-center">
        ...
        <span><b>Copy</b> code for a single palette, or hit "Copy all code" button for all the generated palettes.
        </span>
      </p>
      <button class="rounded mx-auto px-5 py-2 mb-8 flex items-center" @click="copyToClipboard" :disabled="copied"
        :class="copyBtnClass">
      ...
      <div class="flex max-w-6xl mx-auto flex-col md:flex-row">
        <div v-for="color in demo.colors" :key="color.main.hex"
          :class="`md:${demo.colors.length > 1 ? `w-1/${demo.colors.length}` : 'w-full'}`">
      ...
      <div class="w-full flex opacity-0 absolute" id="palettes-code" style="z-index: -1;">
        <tailwind-code v-for="color in demo.colors" :key="color.main.hex" :palette="color.code" :name="color.main.hex"
          class="text-sm m-1" />
      </div>
    </div>
  </div>
